...
interface TransferPayload {
  from: string;
  to: string;
  lines: TransferApiLine[];
}

const newLine = (): TransferLine => ({ name: "", color: "", sku: null, colorOptions: [], sizeQty: {} });
...
export function TransferForm({
  ...
  const patch = (i: number, next: Partial<TransferLine>) =>
  ...
  const submit = () => {
    if (!from.trim()) return setError("From location is required.");
    if (!to.trim()) return setError("To location is required.");
    if (from === to) return setError("From and To must be different locations.");
    const apiLines: TransferApiLine[] = [];
    for (const l of lines) {
      if (!l.name.trim()) continue;
      for (const size of sizes) {
        const q = Math.floor(Number(l.sizeQty[size]) || 0);
        if (q > 0) {
          apiLines.push({ name: l.name.trim(), color: l.color.trim() || null, size, sku: null, qty: q });
        }
      }
    }
    ...
    setError(null);
    onSubmit({ from: from.trim(), to: to.trim(), lines: apiLines });
  };

  const locSelect = (value: string, onChange: (v: string) => void, exclude: string) => (
    <Select value={value} onValueChange={onChange}>
      <SelectTrigger><SelectValue placeholder="Select location" /></SelectTrigger>
      <SelectContent>
        {locations.filter((l) => l.name !== exclude).map((l) => (
      ...
      <div className="erp-scroll flex-1 space-y-4 overflow-y-auto px-6 pb-6">
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1.5">
            <Label>From<span className="ml-0.5 text-brand-orange">*</span></Label>
            {locSelect(from, setFrom, to)}
          </div>
          <div className="space-y-1.5">
            <Label>To<span className="ml-0.5 text-brand-orange">*</span></Label>
            {locSelect(to, setTo, from)}
          </div>
        </div>
        ...
        <div className="flex items-center justify-between rounded-xl bg-muted/60 px-4 py-3">
          <span className="text-[13px] font-semibold text-muted-foreground">Total units</span>
